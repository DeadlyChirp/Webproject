<?php
    ...
    if (isset($_GET["usersEmail"]) && isset($_GET["token"])) {

        $email = $connection->real_escape_string($_GET["usersEmail"]);
        $token = $connection->real_escape_string($_GET["token"]);

        $data = $connection->query("SELECT usersId FROM users WHERE usersEmail='$email' AND token='$token'");
        ...
        if($data->num_rows >0){
            ...
            $connection->query("UPDATE users SET usersPwd='$password', token='' WHERE usersEmail='$email'");
